...
import { Send, ArrowLeft, Search, Plus, MoreVertical, Trash2, Edit2, Check, X, Users } from "lucide-react";
...
interface WhatsAppChatFullscreenProps {
  onClose: () => void;
}

export default function WhatsAppChatFullscreen({ onClose }: WhatsAppChatFullscreenProps) {
  const { user } = useAuth();
        ...
        <div className="w-96 border-r border-slate-200 dark:border-slate-800 flex flex-col">
          ...
          <div className="p-4 border-b border-slate-200 dark:border-slate-800 space-y-3">
              ...
              <div className="flex gap-2">
                <Dialog open={isCreatingGroup} onOpenChange={setIsCreatingGroup}>
                  <DialogTrigger asChild>
                    <Button size="icon" variant="ghost" className="rounded-full" data-testid="button-new-chat-fs">
                    ...
                    <div className="space-y-4">
                      ...
                      {selectedUsers.length > 1 && (
                        <div>
                          ...
                          <Input
                            ...
                            className="mt-1"
                            data-testid="input-group-name-fs"
                          />
                        </div>
                      )}
                      ...
                      <div className="flex gap-2">
                        <Button
                          ...
                          className="flex-1"
                          data-testid="button-start-chat-fs"
                        >
                          Start Chat
                        </Button>
                        <Button
                ...
                <Button size="icon" variant="ghost" onClick={onClose} data-testid="button-close-fs">
            ...
            <Input
              ...
              data-testid="input-search-chat-fs"
            />
          </div>
            ...
            <div className="space-y-1 p-2">
                ...
                <>
                  ...
                  {filteredConversations.map((conv) => (
                    <button
                      ...
                      data-testid={`button-conversation-fs-${conv.id}`}
                    >
                      <div className="font-bold text-sm truncate">{getConversationName(conv)}</div>
                ...
                <>
                  ...
                  {filteredUsers.map((u: any) => (
                    <button
                      ...
                      data-testid={`button-user-fs-${u.id}`}
                    >
                      <div className="font-medium text-sm truncate">{u.username}</div>
                      <div className="text-xs text-slate-500 dark:text-slate-400">Click to chat</div>
                    </button>
                  ))}
                </>
              )}
              ...
              {filteredConversations.length === 0 && filteredUsers.length === 0 && (
                <div className="p-8 text-center text-sm text-slate-500">
                  {searchQuery ? "No chats or users found" : "No chats yet. Click + to start chatting!"}
                </div>
              )}
            </div>
          </ScrollArea>
          ...
          {selectedConvId && selectedConversation ? (
            <>
              ...
              <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-slate-950">
                <div>
                  <h3 className="font-bold text-lg">{getConversationName(selectedConversation)}</h3>
                  ...
                  {messages.map((msg) => (
                    ...
                    >
                          ...
                          editingId === msg.id ? (
                            <div className="flex gap-1">
                              <Input
                                ...
                                data-testid={`input-edit-fs-${msg.id}`}
                              />
                              <Button
                                size="sm"
                                onClick={() => editMutation.mutate({ id: msg.id, message: editText })}
                                className="h-8"
                                data-testid={`button-save-fs-${msg.id}`}
                              >
                            ...
                            <>
                              {!selectedConversation.isGroupChat ? null : (
                                <p className="text-xs opacity-70 mb-1 font-semibold">{msg.user?.username}</p>
                              )}
                      ...
                      {msg.senderId === user?.id && !msg.isDeleted && (
                        <div className="flex gap-1 ml-2 items-center opacity-0 group-hover:opacity-100 transition">
                          <Button
                            size="sm"
                            variant="ghost"
                            className="h-6 w-6"
                            ...
                            data-testid={`button-edit-fs-${msg.id}`}
                          >
                          ...
                          <Button
                            ...
                            data-testid={`button-delete-fs-${msg.id}`}
                          >
              ...
              <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 flex gap-2">
                <Input
                  ...
                  data-testid="input-message-fs"
                />
                <Button
                  onClick={handleSend}
                  disabled={!message.trim() || sendMutation.isPending}
                  data-testid="button-send-fs"
                >
