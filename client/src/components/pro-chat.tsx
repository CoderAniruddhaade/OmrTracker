          ...
          <TabsContent value="whisper" className="flex flex-col flex-1 gap-3">
            <div className="flex gap-2">
              <select
                ...
                data-testid="select-whisper-recipient"
              >
                <option value="">Select recipient...</option>
                {allUsers.map((u: any) => (
                  <option key={u.id} value={u.id}>
                    {u.firstName} {u.lastName} {onlineUserIds.includes(u.id) ? "(Online)" : ""}
                  </option>
                ))}
              </select>
            </div>
            ...
            <ScrollArea className="flex-1 pr-4" ref={scrollRef}>
              <div className="space-y-3">
                {whispers.filter(
                  (w: WhisperMessage) =>
                    w.senderId === user?.id || w.recipientIds.includes(user?.id || "")
                ).length === 0 ? (
                  <p className="text-xs sm:text-sm text-muted-foreground text-center py-8">
                    No whispers yet. Start a private conversation!
                  </p>
                ) : (
                  whispers
                    .filter(
                    ...
                    .map((w: WhisperMessage) => (
                      <div key={w.id} className="text-xs sm:text-sm p-2 rounded bg-accent/10">
                        <div className="flex items-center justify-between mb-1">
                          <span className="font-semibold">
                            {w.senderId === user?.id ? "You" : (w as any).user?.firstName || "User"}
                          </span>
                          <Badge variant="secondary" className="h-4 px-1.5 text-xs">
                            Whisper
                          </Badge>
                        </div>
                        <p className="text-muted-foreground break-words">{w.message}</p>
                        {w.editedAt && <p className="text-xs text-muted-foreground">(edited)</p>}
                        {user?.id === w.senderId && (
                          <div className="flex gap-2 mt-2">
                            <Button
                              size="sm"
                            ...
                            <Button
                              ...
                              variant="ghost"
                              className="h-6"
                              onClick={() => apiRequest("DELETE", `/api/whispers/${w.id}`, {}).then(() => queryClient.invalidateQueries({ queryKey: ["/api/whispers"] }))}
                            >
                              <Trash2 className="w-3 h-3" />
                            </Button>
                          </div>
            ...
            {editingId && (
              <div className="flex gap-2">
                ...
                <Button
                  size="sm"
                  onClick={() =>
                    apiRequest("PATCH", `/api/whispers/${editingId}`, { message: editText }).then(
                      () => {
                        setEditingId(null);
                        setEditText("");
                        queryClient.invalidateQueries({ queryKey: ["/api/whispers"] });
            ...
            <div className="flex gap-2">
              <Input
                ...
                className="text-xs sm:text-sm"
                disabled={!selectedRecipient || whisperMutation.isPending}
              />
              <Button
                size="sm"
                onClick={handleSend}
                disabled={!message.trim() || !selectedRecipient || whisperMutation.isPending}
